...
app.post('/genImage', async (req, res) => {
    try {
        if (!req.body.text) {
            res.status(400).send('Please provide valid text for image generation.');
            return;
        }
        const imagePath = await browserModule.genImage(req.body.text);
        res.send({"imagePath": imagePath});
        // Save cookies since we've definitely logged in
        await browserModule.saveCookies();
    } catch (error) {
        console.error(error);
...
app.post('/uploadImage', async (req, res) => {
    try {
        if (!req.body.path) {
            res.status(400).send('Please provide a valid image path.');
            return;
        }
        await browserModule.uploadImage(req.body.path);
